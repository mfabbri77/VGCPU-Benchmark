...
- [SCOPE-01-01] **Product**: `vgcpu-benchmark` — a cross-platform CLI to benchmark CPU vector-graphics rendering backends by executing canonical IR scenes through per-backend adapters and emitting comparable reports.
...
- [REQ-03] The system MUST execute canonical IR scenes (from the assets manifest) through a selected backend adapter and capture timings per scene.
...
- [REQ-06] The system MUST expose a backend registry:
  - [REQ-06-01] list available backends (`list`)
  - [REQ-06-02] run a backend by name (`run --backend <name>`)
- [REQ-07] Backends MUST be build-time selectable via CMake options.

## [SCOPE-04] Out of scope
- [SCOPE-04-01] GPU acceleration benchmarks (Vulkan/Metal/DX12) — out of scope for v1.0 blueprint.
- [SCOPE-04-02] GUI front-end — out of scope (CLI only).
- [SCOPE-04-03] Distributed benchmarking orchestration (agents, server) — out of scope.
- [SCOPE-04-04] Guaranteeing deterministic results across different machines — out of scope (only “same machine best effort”).

## [SCOPE-05] Backend tiering policy (scope decision)
- [DEC-SCOPE-02] Define **Tier-1** backends for v0.2.0 as:
...
Because this is a benchmark tool, “performance” is mostly about *overhead control* and *CI practicality*.
...
- [DEC-SCOPE-04] Determinism target for CI gates is “coarse”: detect **large regressions** (e.g., ≥10–15%) rather than small deltas.
...
This chapter defines [REQ-*] only. Concrete [TEST-*] IDs and their implementations are specified in Ch8/Ch9, and each [REQ-*] will map to:
- ≥1 [TEST-*] entry, and
